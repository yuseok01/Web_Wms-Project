...
//    }
    ...
    public void save(ProductImportRequestData data, Product product) {
        //product와 requestData를 통해 필요한 데이터를 추출
        Import importData = ImportMapper.fromProduct(product, data.getDate());
...
//        2. {date, List<PIRD>}형태로 묶기
        List<ProductImportRequestData> dataList = new ArrayList<>();
        for (Import imp : importList) {
            ProductImportRequestData productData = ProductImportRequestData.builder()
                .name(imp.getName())
                .quantity(imp.getQuantity())
                .barcode(imp.getBarcode())
                .expiry(imp.getExpirationDate())
